...
def run_smoke(batch: int, m: int, n: int, k: int) -> None:
    ...
    weight = torch.ones((batch, k, n), device=DEVICE, dtype=torch.float8_e5m2)
    weight = weight.transpose(1, 2)
    scale = torch.tensor(1.0, device=DEVICE, dtype=torch.float32)
    out = _batch_dense(grad_output, weight, scale, scale)
    torch.cuda.synchronize()
    print(f"lrm_bmm_grad_activation smoke passed, output_shape={tuple(out.shape)}")


def main():
    parser = argparse.ArgumentParser(description="LRM FP8 BMM grad-activation benchmark.")
