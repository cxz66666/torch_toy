...
def run_smoke(batch: int, m: int, n: int, k: int) -> None:
    ...
    out_scale = torch.tensor(1.0, device=DEVICE, dtype=torch.float32)
    out = _batch_matmul_fwd(a, b.transpose(1, 2).contiguous().transpose(1, 2), out_scale=out_scale)
    torch.cuda.synchronize()
    print(f"FP8 BMM column-major smoke passed, output_shape={tuple(out.shape)}")


def main():
    parser = argparse.ArgumentParser(description="FP8 BMM column-major benchmark.")
