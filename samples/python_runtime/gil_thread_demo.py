...
def red_cpu_task(work_items: int):
    name = "RED_CPU_TASK"
...
def blue_cpu_task(work_items: int):
    name = "BLUE_CPU_TASK"
    print(f"[{name}] Starting...")
    while True:
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Show CPU-bound threads contending for the GIL.")
    parser.add_argument("--seconds", type=float, default=10.0)
    parser.add_argument("--work-items", type=int, default=1_000_000)
    args = parser.parse_args()

    global stop_event
    stop_event = threading.Event()

    t1 = threading.Thread(target=red_cpu_task, args=(args.work_items,))
    t2 = threading.Thread(target=blue_cpu_task, args=(args.work_items,))
    ...
    print(f"Main: Threads are fighting for GIL. Running for {args.seconds:g} seconds...")
