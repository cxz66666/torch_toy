...
configs = [
        ...
        triton.Config({'BLOCK_SIZE_M': 64, 'BLOCK_SIZE_N': 64, 'BLOCK_SIZE_K': 128, 'GROUP_SIZE_M': 1}, num_stages=3, num_warps=8),
        triton.Config({'BLOCK_SIZE_M': 64, 'BLOCK_SIZE_N': 128, 'BLOCK_SIZE_K': 64, 'GROUP_SIZE_M': 1}, num_stages=3, num_warps=4),
        triton.Config({'BLOCK_SIZE_M': 64, 'BLOCK_SIZE_N': 128, 'BLOCK_SIZE_K': 64, 'GROUP_SIZE_M': 4}, num_stages=3, num_warps=4),
        triton.Config({'BLOCK_SIZE_M': 64, 'BLOCK_SIZE_N': 128, 'BLOCK_SIZE_K': 64, 'GROUP_SIZE_M': 8}, num_stages=3, num_warps=8),
        triton.Config({'BLOCK_SIZE_M': 64, 'BLOCK_SIZE_N': 32, 'BLOCK_SIZE_K': 128, 'GROUP_SIZE_M': 4}, num_stages=4, num_warps=4),
],
    key=['M', 'N', 'K'],
)
@triton.jit
...
def benchmark(B, M, N, K, provider):
    ...
    b = b.transpose(1, 2).contiguous().transpose(1, 2)  # Ensure B is in column-major order
    ...
    if provider == "always_nt":        
        ms, min_ms, max_ms = triton.testing.do_bench(lambda: _batch_dense(a, b, scale_a, scale_b), quantiles=quantiles)
    perf = lambda ms: 2 * B * M * N * K * 1e-12 / (ms * 1e-3)
    return perf(ms), perf(max_ms), perf(min_ms)


def run_smoke(batch: int, m: int, n: int, k: int) -> None:
    if not torch.cuda.is_available():
        raise RuntimeError("This sample requires CUDA.")

    a = torch.ones((batch, m, k), device=DEVICE, dtype=torch.float8_e5m2)
    b = torch.ones((batch, k, n), device=DEVICE, dtype=torch.float8_e5m2)
    b = b.transpose(1, 2).contiguous().transpose(1, 2)
    scale = torch.tensor(1.0, device=DEVICE, dtype=torch.float32)
    out = _batch_dense(a, b, scale, scale)
    torch.cuda.synchronize()
    print(f"lrm_bmm_forward smoke passed, output_shape={tuple(out.shape)}")


def main():
    parser = argparse.ArgumentParser(description="LRM FP8 BMM forward benchmark.")
    ...
    if args.smoke:
        run_smoke(args.batch, args.m, args.n, args.k)
    else:
