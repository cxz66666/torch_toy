...
int main(int argc, char** argv){
  ...
  unsigned long max_iters = 1000;
  if (argc > 1) {
    max_iters = std::strtoul(argv[1], nullptr, 10);
  }
  ...
  std::cout << "Completed bounded sync experiment: iterations=" << i
            << ",bad_apple=" << bad_apple_cnt
            << ",same_sm=" << same_sm << std::endl;
  return bad_apple_cnt == 0 ? 0 : 1;
}
