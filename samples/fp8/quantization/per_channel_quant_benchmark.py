...
def run_smoke(batch: int, m: int, n: int) -> None:
    if not torch.cuda.is_available():
        raise RuntimeError("This sample requires CUDA.")

    x = torch.randn((batch, m, n), device=DEVICE, dtype=torch.bfloat16)
    for axiswise_dim in (-1, -2):
        y_ref, scale_ref = fp8_quant(x, torch.float8_e4m3fn, axiswise_dim)
        y_triton, scale_triton = fp8_quant_triton_block(
            x, torch.float8_e4m3fn, axiswise_dim
        )
        scale_ok = torch.allclose(scale_ref, scale_triton)
        value_ok = torch.allclose(y_ref.to(torch.bfloat16), y_triton.to(torch.bfloat16))
        print(f"axiswise_dim={axiswise_dim}: scale_ok={scale_ok}, value_ok={value_ok}")
        if not scale_ok or not value_ok:
            raise AssertionError(f"FP8 per-channel quant smoke failed for axis {axiswise_dim}")


def main():
    parser = argparse.ArgumentParser(description="FP8 per-channel quantization benchmark.")
    parser.add_argument("--smoke", action="store_true", help="Run a small correctness smoke instead of the benchmark.")
    ...
    if args.smoke:
        run_smoke(args.batch, args.m, args.n)
    else:
