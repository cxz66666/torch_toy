...
def fp8_per_tensor_quant_bmm_persistent_tma(
    ...
    if TensorDescriptor is None:
        raise RuntimeError(
            "This custom host TensorDescriptor sample requires triton.tools.tensor_descriptor, "
            "which is not available in this Triton build."
        )
...
HAS_HOST_TENSOR_DESC = supports_tma() and TensorDescriptor is not None
HAS_WARP_SPECIALIZE = supports_ws() and HAS_TENSOR_DESC


def matmul_get_configs(pre_hook=None):
    if os.getenv("TORCH_TOY_TMA_FULL_AUTOTUNE") != "1":
        return [
            triton.Config(
...
if __name__ == "__main__":
    ...
    parser.add_argument("--mode", choices=["custom-bmm", "tutorial"], default="custom-bmm")
    ...
    if args.mode == "custom-bmm":
        if not HAS_HOST_TENSOR_DESC:
            print("Skipping custom host TensorDescriptor BMM: this Triton build does not provide TensorDescriptor.")
            raise SystemExit(0)
        bench_fp8_bmm_tma(
            batch=args.batch,
            m=args.m,
            n=args.n,
            k=args.K,
            warmup_iter=args.warmup_reps,
            benchmark_iter=args.reps,
        )
    elif not HAS_TENSOR_DESC:
        print("Skipping tutorial TMA matmul: this Triton build does not provide tl.make_tensor_descriptor.")
        raise SystemExit(0)
    elif args.prec == 'fp8' and (not hasattr(torch, "float8_e4m3fn") or not is_cuda()):
        ...
        for K in range(args.K_range[0], args.K_range[1] + 1, args.K_step):
            bench(K, dtype, reps=args.reps, warmup_reps=args.warmup_reps)
        proton.finalize()
