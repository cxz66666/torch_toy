...
def triton_fp8_quant(x, use_tma: bool = False):
    x_flat = x.contiguous().view(-1)
    n = x_flat.numel()
    partial_max = torch.empty(32*1024*4, device=x.device, dtype=x.dtype)
    kernel = _block_reduce_minmax_kernel_tma if use_tma else _block_reduce_minmax_kernel
    kernel[(lambda meta: (triton.cdiv(n, meta['BLOCK_SIZE']),))](
        x_flat, n,
        partial_max,
    )
    return partial_max
...
def main():
    parser = argparse.ArgumentParser(description="FP8 per-tensor quantization NCU sample")
    ...
    parser.add_argument(
        "--use-tma",
        action="store_true",
        help="Use the experimental descriptor-load variant. The default uses regular tl.load for a portable smoke.",
    )
    ...
    x = torch.randn((args.batch, args.m, args.n), device="cuda", dtype=torch.bfloat16)
    partial_max = triton_fp8_quant(x, use_tma=args.use_tma)
    torch.cuda.synchronize()
    print(f"finished per-tensor quant smoke for shape={tuple(x.shape)}, partial_shape={tuple(partial_max.shape)}")
