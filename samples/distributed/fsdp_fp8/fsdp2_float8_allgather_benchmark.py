import torch
import torch.nn as nn
import torch.optim as optim
import torch.distributed as dist
from torch.distributed.device_mesh import init_device_mesh
from torch.distributed.fsdp import fully_shard, MixedPrecisionPolicy
from torch.utils.data import Dataset, DataLoader
import os
import time
from collections import OrderedDict

try:
    from torchinfo import summary
except ModuleNotFoundError:
    summary = None


class LinearBlock(nn.Module):
    def __init__(self, hidden_size: int, intermediate_size: int):
        super().__init__()
        self.linear1 = nn.Linear(hidden_size, intermediate_size).to(torch.bfloat16)
        self.linear2 = nn.Linear(intermediate_size, hidden_size).to(torch.bfloat16)
        self.linear3 = nn.Linear(hidden_size, intermediate_size).to(torch.bfloat16)
        self.linear4 = nn.Linear(intermediate_size, hidden_size).to(torch.bfloat16)

    def forward(self, x):
        x = self.linear1(x)
        x = self.linear2(x)
        x = self.linear3(x)
        x = self.linear4(x)
        return x

class MyModel(nn.Module):
    def __init__(self, num_blocks=32, hidden_size: int = 4096, intermediate_size: int = 32768):
        super().__init__()
        layers = [LinearBlock(hidden_size, intermediate_size) for _ in range(num_blocks)]
        self.fc_layers = nn.Sequential(*layers)
    @property
    def layers(self) -> OrderedDict :
        return self.fc_layers._modules

    def forward(self, x):
        return self.fc_layers(x)

# 3. 构造模拟训练数据
class DummyDataset(Dataset):
    def __init__(self, num_samples, input_features, output_features):
        self.num_samples = num_samples
        self.input_features = input_features
        self.output_features = output_features

    def __len__(self):
        return self.num_samples

    def __getitem__(self, idx):
        data = torch.randn(self.input_features)
        target = torch.randn(self.output_features) # 假设目标与模型输出维度一致
        return data, target

def get_model_parameter_count(model: nn.Module, count_trainable_only: bool = False) -> int:
    total_params = 0
    for param in model.parameters():
        if count_trainable_only and not param.requires_grad:
            continue
        total_params += param.numel()
    return total_params


def module_filter_fn(mod: nn.Module, fqn: str) -> bool:
    if not isinstance(mod, nn.Linear):
        return False
        # All dims must be divisible by 16 due to float8 tensorcore hardware requirements.
    dims_multiples_of_16 = (
        mod.weight.shape[0] % 16 == 0 and mod.weight.shape[1] % 16 == 0
    )

    filter23 = "linear2" in fqn or "linear3" in fqn or "linear" in fqn

    if dims_multiples_of_16 and filter23:
        print(f"Swapping {fqn} to Float8Linear")
    return dims_multiples_of_16 and filter23

def load_float8_helpers():
    try:
        from torchao.float8 import (
            Float8LinearConfig,
            convert_to_float8_training,
            precompute_float8_dynamic_scale_for_fsdp,
        )
    except ModuleNotFoundError as exc:
        raise RuntimeError(
            "FP8 mode requires torchao. Install torchao in this environment or run without --fp8."
        ) from exc
    return (
        Float8LinearConfig,
        convert_to_float8_training,
        precompute_float8_dynamic_scale_for_fsdp,
    )


def precompute_float8_dynamic_scale_hook(model: nn.Module | list[nn.Module], precompute_fn):
    models = [model] if isinstance(model, nn.Module) else model
    for m in models:
        precompute_fn(m)


def str_to_bool(value: str | bool) -> bool:
    if isinstance(value, bool):
        return value
    normalized = value.lower()
    if normalized in {"1", "true", "t", "yes", "y"}:
        return True
    if normalized in {"0", "false", "f", "no", "n"}:
        return False
    raise ValueError(f"Expected a boolean value, got {value!r}")


def setup(rank, world_size):
    print(f"Setting up process group for rank {rank} out of {world_size}")
    local_rank = int(os.environ.get("LOCAL_RANK", rank))
    torch.cuda.set_device(local_rank)
    dist.init_process_group(backend="nccl", device_id=torch.device(local_rank),)
    print("Process group initialized.")

def cleanup():
    dist.destroy_process_group()

def fsdp_main(rank, world_size, args):
    setup(rank, world_size)
    fp8_helpers = load_float8_helpers() if args.fp8 or args.precompute_scale else None

    model = MyModel(
        num_blocks=args.num_blocks,
        hidden_size=args.hidden_size,
        intermediate_size=args.intermediate_size,
    )
    # model = model.to(rank) # 将模型移动到当前 rank 的 GPU

    if rank == 0:
        total_params_base = get_model_parameter_count(model)
        print(f"Total parameters (base model): {total_params_base / 1e6:.2f}M")
        total_params_base_trainable = get_model_parameter_count(model,count_trainable_only=True)
        print(f"Total trainable parameters (base model): {total_params_base_trainable / 1e6:.2f}M")


    if args.fp8:
        Float8LinearConfig, convert_to_float8_training, _ = fp8_helpers
        fp8_config = Float8LinearConfig(
            enable_fsdp_float8_all_gather=args.fp8_all_gather,
            force_recompute_fp8_weight_in_bwd=args.force_recompute,
        )
        convert_to_float8_training(
            model,
            config=fp8_config,
            module_filter_fn=module_filter_fn,
        )
        if rank == 0:
            total_params_fp8 = get_model_parameter_count(model)
            print(f"Total parameters  (FP8): {total_params_fp8 / 1e6:.2f}M")
            total_params_fp8_trainable = get_model_parameter_count(model, count_trainable_only=True)
            print(f"Total trainable parameters (FP8): {total_params_fp8_trainable / 1e6:.2f}M")


    mesh = init_device_mesh("cuda", [world_size], mesh_dim_names=["dp_shard"])
    mp_policy = MixedPrecisionPolicy(param_dtype=torch.bfloat16, reduce_dtype=torch.bfloat16, cast_forward_inputs=True, output_dtype=None)
    fsdp_config = {"mesh": mesh, "mp_policy": mp_policy}

    for layer_id, transformer_block in model.layers.items():
        reshard_after_forward = int(layer_id) < len(model.layers) - 1
        fully_shard(
            transformer_block,
            **fsdp_config,
            reshard_after_forward=reshard_after_forward,
        )
    fsdp_model = fully_shard(model, **fsdp_config, reshard_after_forward=True)

    if rank == 0:
        total_params_fsdp = get_model_parameter_count(fsdp_model)
        print(f"Total parameters (FSDP model): {total_params_fsdp / 1e6:.2f}M")

        total_params_fsdp_trainable = get_model_parameter_count(fsdp_model, count_trainable_only=True)
        print(f"Total trainable parameters (FSDP model): {total_params_fsdp_trainable / 1e6:.2f}M")

        torch.cuda.synchronize(torch.cuda.current_device()) # 确保所有操作完成
        allocated_memory_fsdp_rank = torch.cuda.memory_allocated(torch.cuda.current_device()) / (1024**2)
        reserved_memory_fsdp_rank = torch.cuda.memory_reserved(torch.cuda.current_device()) / (1024**2)

        print(f"\n--- GPU Memory Usage on Rank {rank} (FSDP Model) ---")
        print(f"Memory Allocated: {allocated_memory_fsdp_rank:.2f} MB")
        print(f"Memory Reserved: {reserved_memory_fsdp_rank:.2f} MB")

    
    if rank == 0 and summary is not None:
        summary(fsdp_model, input_size=(args.batch_size, args.hidden_size))
    elif rank == 0:
        print("torchinfo is not installed; skipping model summary.")

    # 优化器 (必须在模型 FSDP 包装之后创建)
    optimizer = optim.Adam(fsdp_model.parameters(), lr=args.lr)

    if args.precompute_scale:
        _, _, precompute_float8_dynamic_scale_for_fsdp = fp8_helpers
        optimizer.register_step_post_hook(
            lambda *args, **kwargs:
             precompute_float8_dynamic_scale_hook(fsdp_model, precompute_float8_dynamic_scale_for_fsdp)
        )
        print("Precomputing float8 dynamic scale for FSDP")

    # 损失函数
    criterion = nn.MSELoss()

    dataset = DummyDataset(args.num_samples_per_epoch, args.hidden_size, args.hidden_size)
    # 使用 DistributedSampler 来确保数据在不同 GPU 之间正确划分
    train_sampler = torch.utils.data.distributed.DistributedSampler(
        dataset,
        num_replicas=world_size,
        rank=rank,
        shuffle=True # 通常训练时需要打乱
    )
    train_loader = DataLoader(
        dataset,
        batch_size=args.batch_size,
        sampler=train_sampler,
        num_workers=args.num_workers, # 根据系统调整
        pin_memory=True
    )

    nsys_profiler = torch.cuda.cudart()
    # 训练循环
    fsdp_model.train()
    for epoch in range(args.epochs):
        if epoch==3:
            nsys_profiler.cudaProfilerStart()
        if epoch == 4:
            nsys_profiler.cudaProfilerStop()
        epoch_loss = 0.0
        epoch_start_time = time.time()
        num_batches = 0
        if train_sampler: # 如果使用了sampler，需要在每个epoch开始时设置
            train_sampler.set_epoch(epoch)

        torch.distributed.barrier() # 确保所有进程都准备好开始训练

        for batch_idx, (data, target) in enumerate(train_loader):
            device = torch.cuda.current_device()
            data = data.to(torch.bfloat16).to(device)
            target = target.to(torch.bfloat16).to(device)

            optimizer.zero_grad()
            output = fsdp_model(data)
            loss = criterion(output, target)
            loss.backward()
            optimizer.step()

            epoch_loss += loss.item()
            num_batches += 1

            if rank == 0 and (batch_idx % args.log_interval == 0):
                print(f"Epoch: {epoch+1}/{args.epochs} | Batch: {batch_idx}/{len(train_loader)} | Loss: {loss.item():.4f}")

        epoch_duration = time.time() - epoch_start_time
        avg_epoch_loss = epoch_loss / num_batches if num_batches > 0 else 0
        samples_per_second = (args.num_samples_per_epoch / world_size * world_size) / epoch_duration if epoch_duration > 0 else 0


        avg_loss_tensor = torch.tensor([avg_epoch_loss], device=torch.cuda.current_device())
        dist.all_reduce(avg_loss_tensor, op=dist.ReduceOp.AVG) # 计算所有 rank 的平均 loss

        if rank == 0:
            print(f"-" * 50)
            print(f"Epoch {epoch+1} Summary:")
            print(f"  Average Loss: {avg_loss_tensor.item():.4f}")
            print(f"  Epoch Duration: {epoch_duration:.2f} seconds")
            print(f"  Samples/sec: {samples_per_second:.2f}")
            print(f"-" * 50)

    cleanup()

if __name__ == '__main__':
    import argparse
    parser = argparse.ArgumentParser(description='PyTorch FSDP Linear Model Benchmark')
    parser.add_argument('--batch_size', type=int, default=16, # 根据显存调整
                        help='input batch size for training (default: 16)')
    parser.add_argument('--epochs', type=int, default=5, # 简单测试，少量 epoch
                        help='number of epochs to train (default: 2)')
    parser.add_argument('--lr', type=float, default=0.001,
                        help='learning rate (default: 0.001)')
    parser.add_argument('--num_blocks', type=int, default=16,
                        help='number of LinearBlock layers in the model (default: 4)')
    parser.add_argument('--hidden_size', type=int, default=4096,
                        help='hidden dimension for each linear block')
    parser.add_argument('--intermediate_size', type=int, default=32768,
                        help='intermediate dimension for each linear block')
    parser.add_argument('--num_samples_per_epoch', type=int, default=2048, # 总样本数
                        help='number of samples per epoch (default: 8000)')
    parser.add_argument('--log_interval', type=int, default=10,
                        help='how many batches to wait before logging training status')
    parser.add_argument('--num_workers', type=int, default=2, # DataLoader workers
                        help='number of dataloader workers (default: 2)')
    parser.add_argument('--fp8', type=str_to_bool, default=False,
                        help='swap nn.Linear with Float8Linear to perform float8 matmul')
    parser.add_argument('--fp8_all_gather', type=str_to_bool, default=False,
                        help='whether to use float8 all-gather')
    parser.add_argument('--precompute-scale', '--precompute_sclae', dest='precompute_scale',
                        type=str_to_bool, default=False,
                        help='communicate AMAX/scales efficiently in a single all-reduce for all parameters instead of doing many small all-reduce for each parameter')
    parser.add_argument('--force_recompute', type=str_to_bool, default=False,
                        help='force recomputation of fp8 weights during backward pass, preventing unsharded fp8 weights from being saved for backward')


    args = parser.parse_args()

    # torchrun 会自动设置这些环境变量
    world_size = int(os.environ.get("WORLD_SIZE", 1))
    rank = int(os.environ.get("RANK", 0)) # 全局 rank
    local_rank = int(os.environ.get("LOCAL_RANK", 0)) # 节点内 rank

    # FSDP通常使用全局 rank
    # 如果 world_size > 1 (即 torchrun 启动了多个进程)
    if world_size > 1:
        fsdp_main(rank=rank, world_size=world_size, args=args)
    else: # 单进程运行 (例如调试，不使用 FSDP)
        print("Running in single process mode (no FSDP).")
        if torch.cuda.is_available():
            fsdp_main(rank=0, world_size=1, args=args)
        else:
            print("CUDA not available. FSDP example requires CUDA.")
