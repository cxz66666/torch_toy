...
def run_smoke(batch: int, m: int, n: int, k: int) -> None:
    ...
    grad_output = torch.ones((batch, m, n), device=DEVICE, dtype=torch.float8_e5m2)
    activation = torch.ones((batch, m, k), device=DEVICE, dtype=torch.float8_e5m2)
    scale = torch.tensor(1.0, device=DEVICE, dtype=torch.float32)
    out = _batch_db_nt(grad_output, activation, scale, scale)
    torch.cuda.synchronize()
    print(f"lrm_bmm_grad_weight smoke passed, output_shape={tuple(out.shape)}")


def main():
    parser = argparse.ArgumentParser(description="LRM FP8 BMM grad-weight benchmark.")
