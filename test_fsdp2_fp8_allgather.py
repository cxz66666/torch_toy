...
class LinearBlock(nn.Module):
    def __init__(self):
        super().__init__()
        self.linear1 = nn.Linear(4096, 32768).to(torch.bfloat16)
        self.linear2 = nn.Linear(32768, 4096).to(torch.bfloat16)
        self.linear3 = nn.Linear(4096, 32768).to(torch.bfloat16)
        self.linear4 = nn.Linear(32768, 4096).to(torch.bfloat16)
...
def precompute_float8_dynamic_scale_hook(model: nn.Module | list[nn.Module]):
...
def setup(rank, world_size):
    ...
    dist.init_process_group(backend="nccl", device_id=torch.device(rank),)
...
def fsdp_main(rank, world_size, args):
    ...
    if args.precompute_sclae:
        optimizer.register_step_post_hook(
            lambda *args, **kwargs:
             precompute_float8_dynamic_scale_hook(fsdp_model)
        )
...
if __name__ == '__main__':
    ...
    parser.add_argument('--fp8_all_gather', type=bool, default=False,
                        help='whether to use float8 all-gather')
    parser.add_argument('--precompute_sclae', type=bool, default=False,
