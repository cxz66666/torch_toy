...
def main():
    parser = argparse.ArgumentParser(description="Compare a custom Triton FP8 cast with torch.compile.")
    parser.add_argument("--batch", type=int, default=32)
    parser.add_argument("--m", type=int, default=2240)
    parser.add_argument("--n", type=int, default=1120)
    parser.add_argument("--warmup", type=int, default=100)
    parser.add_argument("--reps", type=int, default=10)
    parser.add_argument("--dtype", choices=["e4m3", "e5m2"], default="e5m2")
    args = parser.parse_args()

    B, M, N = args.batch, args.m, args.n
    DTYPE = torch.float8_e4m3fn if args.dtype == "e4m3" else torch.float8_e5m2
    ...
    scale_close = math.isclose(scale_orig.item(), scale_opt.item(), rel_tol=1e-6)
